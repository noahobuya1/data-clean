...
---
title: 'Module 4: Probabilistic Blocking, Part I'
...
p_load(RecordLinkage,blink,knitr,cora,ggplot2,
       textreuse,tokenizers)
```
...
$$\mid S \cap T\mid =  \;\; \text{the number of members shared between sets} \;\; S,T$$ and 
$$\mid S \cup T \mid = \;\; \text{the total number of members in both sets} \;\; S,T.$$
...
# get n-shingles for a string x
tokenize_character_shingles(x, n)

# calculate jaccard similarity for two sets a,b
...
**Finish the rest of the exercise with a partner, which can be found after the summary**
...
```{r your-turn2-plot, fig.cap="Jaccard similarity for each pair of records. Light blue indicates the two records are more similar and dark blue indicates less similar.", echo=FALSE}
...
- Are many records similar according to the Jaccard similarity plot (visually)?  
- What is problematic in using shingling and the Jaccard similarity (or any sim. score) as a type of blocking?
- Next time, we will consider locality sensitive hashing. 

# Takeaways

- Shingling
- Jaccard similarity
- Understanding why these are not practically useful when considering large data sets. 
